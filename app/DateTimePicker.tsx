...
const convertToRangeValue = (values: [Dayjs | null, Dayjs | null]): RangeValue => {
  return values && values[0] && values[1] ? [values[0], values[1]] : null;
};

const DateTimePicker: React.FC<DateTimePickerProps> = ({ value, onChange }) => {
  const [pickerValue, setPickerValue] = useState<RangeValue>(convertToRangeValue(value || [null, null]));

  useEffect(() => {
    setPickerValue(convertToRangeValue(value));
  }, [value]);

  const handleOnChange = (values: RangeValue, dateStrings: [string, string]) => {
    const convertedValues: [Dayjs | null, Dayjs | null] = values ? [values[0], values[1]] : [null, null];
